...
list_location{1,1}='/Volumes/san256/users_for_mac_system_macPro/jeannerat/Dropbox/damien_marion/compounds/working_directory_jeannerat/';
...
for super_loop_over_location=1:size(list_location,2)
    ...
    for looop_test_zip_sdf=[0 1]%1 ziped   0 .sdf
        if looop_test_zip_sdf==1
            F = dir([  folder_location    '*.zip'] );
        else
              F = dir([  folder_location    '*.sdf'] );
          %  F = dir([  folder_location    'Menthol_explicit_ax_eq_full_assignments_1.nmredata.sdf'] );
        end
        for ii = 1:length(F)
            ...
            for loop_over_compound_of_record = 1:length(F2)
                part=F2(loop_over_compound_of_record).name(1:end);
                ...
                if isfield(super_obj{1}.structure,'nb_bond_between')
                    ...
                    if opt.generate_eps_files
                        ...
                        if  exist('OCTAVE_VERSION', 'builtin') ~= 0
                            print(['./plots_of_figures/O_' dataset_name '_compound' num2str(loop_over_compound_of_record) '_structure.eps'],'-color');%for octave
                        else
                            print(['./plots_of_figures/M_' dataset_name '_compound' num2str(loop_over_compound_of_record) '_structure.eps'],'-depsc');%for matlab
                        end
                    end
                ...
                if ~opt.only_read_data
                                ...
                                if isfield(super_obj{loo},'nb_dim')
                                    ...
                                    if super_obj{loo}.nb_dim == 1
                                        ...
                                        if exist([folder_location_full actualfile '/1r'],'file')
                                            ...
                                            figure(loo);clf;
                                            if opt.plot_1D
                                                plot_1d_spectrum(super_obj{loo}.spectrum,0,[],[],loo);
                                            end
                    ...
                    %% here display coupling information (if any...)
                    for loo=3:size(super_obj,2)%search all spectral objects (assignment is object 1 and J is object 2)
                        sta_num=998;figure(sta_num+loo);clf
                        couter_of_displayed_j=plot_1D_spectra_J_data(super_obj{loo},sta_num+loo,used_color,opt);
                        if couter_of_displayed_j<2
                            close(sta_num+loo);
                        else
                            mkdir('./plots_of_figures')
                            if  exist('OCTAVE_VERSION', 'builtin') ~= 0
                                print(['./plots_of_figures/O_' dataset_name '_compound' num2str(loop_over_compound_of_record) '_'  super_obj{loo}.tag_name '_couplings.eps'],'-color');%for octave
                            else
                                print(['./plots_of_figures/M_' dataset_name '_compound' num2str(loop_over_compound_of_record) '_'  super_obj{loo}.tag_name '_couplings.eps'],'-depsc');%for matlab
                            end
                        end
                    end
                    ...
                    for loop_over_spectra=1:pointer_1d-1
                        ...
                        if opt.generate_eps_files
                            mkdir('./plots_of_figures')
                            if  exist('OCTAVE_VERSION', 'builtin') ~= 0
                                print(['./plots_of_figures/O_' dataset_name '_compound' num2str(loop_over_compound_of_record) '_'  super_obj{list_1d(loop_over_spectra,1)}.tag_name '.eps'],'-color');%for octave
                            else
                                print(['./plots_of_figures/M_' dataset_name '_compound' num2str(loop_over_compound_of_record) '_'  super_obj{list_1d(loop_over_spectra,1)}.tag_name '.eps'],'-depsc');%for matlab
                            end
                        end
                    end
                    %% plot 2D NMReDATA
                    for loop_over_spectra=1:pointer_2d-1
                        ...
                        if opt.generate_eps_files
                            mkdir('./plots_of_figures')
                            if  exist('OCTAVE_VERSION', 'builtin') ~= 0
                                print(['./plots_of_figures/O_' dataset_name '_compound' num2str(loop_over_compound_of_record) '_'  super_obj{list_2d(loop_over_spectra,1)}.tag_name '.eps'],'-color');%for octave
                            else
                                print(['./plots_of_figures/M_' dataset_name '_compound' num2str(loop_over_compound_of_record) '_'  super_obj{list_2d(loop_over_spectra,1)}.tag_name '.eps'],'-depsc');%for matlab
                            end
                        end
                    end
                end
