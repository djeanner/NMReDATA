function obj=extract_data_signals(obj,current_line,pieces_of_information,field_separator,verbose)
if nargin<5
    verbose=0;
end
...
if size(list_separator,2)<2%for 3
    ...
    if verbose
    disp(['Signal with label: "' label_or_chemshift '" at ' num2str(value) ' ppm corresponds to:'])
    end  
...
%             end
      ...
      %  disp(['here is  <' current_string(1,1) '>'])
        if strcmp(current_string(1),'H')
            ...
            if verbose
             disp(['Implicit H atom bound to atom number: ' num2str(atomnum)  ])
            end
        else
           ...
           %     disp(['Explicit' num2str(loop_over_atoms-1) ': bound to atom ' num2str(atomnum) ])
            ...
            if verbose
             disp(['Atom number: ' num2str(atomnum)  ])
            end
        end
